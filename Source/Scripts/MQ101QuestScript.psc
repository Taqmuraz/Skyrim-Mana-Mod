...
function AddRaceSpells()
	...
	Debug.MessageBox("health is set to " + health + "\nstamina is set to " + stamina + "\nmagicka is set to " + magicka)
...
; 	debug.trace(self + " removing all race spells")
	...
	pActor.RemoveSpell(Flames)
	pActor.RemoveSpell(Healing)    
	pActor.RemoveSpell(Frost)  
endFunction
